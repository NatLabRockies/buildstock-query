SELECT baseline."in.comstock_building_type" AS comstock_building_type, sum(1) AS sample_count, sum(baseline.weight) AS units_count, sum(CASE WHEN (CAST(upgrade.applicability AS VARCHAR) = 'true') THEN upgrade."out.electricity.total.energy_consumption..kwh" ELSE baseline."out.electricity.total.energy_consumption..kwh" END * baseline.weight) AS "electricity.total.energy_consumption..kwh" 
FROM (SELECT * 
