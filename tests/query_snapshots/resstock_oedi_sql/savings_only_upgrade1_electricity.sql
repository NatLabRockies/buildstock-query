SELECT baseline."in.geometry_building_type_recs" AS geometry_building_type_recs, sum(1) AS sample_count, sum(baseline.weight) AS units_count, sum((coalesce(baseline."out.electricity.total.energy_consumption", 0) - coalesce(upgrade."out.electricity.total.energy_consumption", 0)) * baseline.weight) AS "electricity.total.energy_consumption__savings" 
FROM (SELECT * 
