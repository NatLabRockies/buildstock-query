SELECT baseline."in.geometry_building_type_recs" AS geometry_building_type_recs, sum(1) AS sample_count, sum(baseline.weight) AS units_count, sum(CASE WHEN (upgrade.applicability = true) THEN upgrade."out.electricity.total.energy_consumption" ELSE baseline."out.electricity.total.energy_consumption" END * baseline.weight) AS "electricity.total.energy_consumption" 
FROM (SELECT * 
